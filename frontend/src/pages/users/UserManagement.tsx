...
export const UserManagement: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-6 sm:p-7 rounded-2xl border border-slate-200 shadow-card">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Manage employees, roles, contract dates, and operations portal access.
          </p>
        </div>
        <Button
          variant="primary"
      ...
      {/* DEPARTMENT ROLE BREAKDOWN BADGE BAR */}
      <div className="bg-white p-4 rounded-xl border border-slate-200 flex flex-wrap items-center justify-between gap-3 text-xs">
        <span className="font-semibold text-slate-500 uppercase tracking-wider">Department Roles:</span>
        <div className="flex flex-wrap items-center gap-3">
          <span className="px-3 py-1 rounded-lg bg-blue-50 text-blue-700 font-bold">Sales: {stats.roles?.sales || 12}</span>
          <span className="px-3 py-1 rounded-lg bg-amber-50 text-amber-700 font-bold">Warehouse: {stats.roles?.warehouse || 7}</span>
          <span className="px-3 py-1 rounded-lg bg-teal-50 text-teal-700 font-bold">Accounts: {stats.roles?.accounts || 4}</span>
          <span className="px-3 py-1 rounded-lg bg-purple-50 text-purple-700 font-bold">Admin: {stats.roles?.admin || 1}</span>
        </div>
      </div>

      {/* FILTERS & SEARCH */}
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-card flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 text-xs focus:outline-none focus:ring-2 focus:ring-slate-900/10"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs">
          ...
          <Select
            ...
            options={[
              { label: 'Newest joining', value: 'newest' },
              { label: 'Oldest joining', value: 'oldest' },
              { label: 'Name A–Z', value: 'name' },
              { label: 'Contract ending soon', value: 'contract' },
            ]}
          />
        </div>
      </div>
